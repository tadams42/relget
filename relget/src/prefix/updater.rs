...
pub(super) fn update(
    ...
    let to_update: Vec<String> = if apps.is_empty() && configured_set.is_none() {
        ...
        let ids = resolve_update_targets(&installed_binaries, Registry::entries());
...
/// prefix is targeted.
fn resolve_update_targets(installed_binaries: &HashSet<&str>, entries: &[AppEntry]) -> Vec<String> {
    entries
        .iter()
        .filter(|e| {
...
mod tests {
    ...
    use super::{filter_to_installed, resolve_update_targets};
